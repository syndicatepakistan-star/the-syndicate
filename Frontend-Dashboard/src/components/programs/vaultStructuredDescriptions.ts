...
const PACK_BODY: Record<VaultPackKey, { hook: string; protocol: string }> = {
  ...
  trading_technical_analysis: {
    ...
    protocol: `${TRADING_PACK_DESCRIPTION} This vault deploys as modular warfare: Scalpel Protocol on the one-minute chart, Strategies of a Master Trader, Setups of a Master Trader, and Secrets of a Master Trader — each unlockable as a $99 module sub-pack or as a full pack. Buy the complete matrix for $150 or purchase module sub-packs à la carte. Individual lessons are included with each module (not sold separately). Every transaction is a single checkout with permanent billing record under your Syndicate identity. Curriculum access activates as modules deploy. No recurring fees. No fake signals. Only controlled entitlement to professional-grade technical analysis built for operators who refuse to be exit liquidity.`,
  },
};
