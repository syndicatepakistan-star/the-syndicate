...
const STRATEGIES_SLUG_TO_KEY: Readonly<Record<string, string>> = {
  trading_master_strategies: "__module_pack__",
  trading_strategies_01: "Strategies of a Master Trader — Lesson",
  trading_strategies_02: "The Keys to Building Wealth",
  trading_strategies_03: "Favorite Trading Indicators",
  trading_strategies_04: "Charting Strategies for Indicators",
  trading_strategies_05: "Support & Resistance Strategies",
  trading_strategies_06: "Candlestick Trading Strategies",
  trading_strategies_07: "Risk vs Rewards & Rules to Trade",
  trading_strategies_08: "Extract the Market Capital",
};

const STRATEGIES_TITLE_TO_KEY: Readonly<Record<string, string>> = {
  [TRADING_STRATEGIES_MODULE_TITLE]: "__module_pack__",
  "the keys to building wealth": "The Keys to Building Wealth",
  "favorite trading indicators": "Favorite Trading Indicators",
  "favourite trading indicators": "Favorite Trading Indicators",
  "charting strategies for indicators": "Charting Strategies for Indicators",
  "support resistance strategies": "Support & Resistance Strategies",
  "support & resistance strategies": "Support & Resistance Strategies",
  "candlestick trading strategies": "Candlestick Trading Strategies",
  "risk vs rewards rules to trade": "Risk vs Rewards & Rules to Trade",
  "risk vs rewards & rules to trade": "Risk vs Rewards & Rules to Trade",
  "extract the market capital": "Extract the Market Capital",
};
...
export function curatedTradingStrategiesDescription(
  ...
  if (slugKey && STRATEGIES_SLUG_TO_KEY[slugKey]) {
    return TRADING_STRATEGIES_PROGRAM_DESCRIPTIONS[STRATEGIES_SLUG_TO_KEY[slugKey]];
  }
  const titleKey = normalizeTitleKey(title ?? "");
  if (titleKey && STRATEGIES_TITLE_TO_KEY[titleKey]) {
    return TRADING_STRATEGIES_PROGRAM_DESCRIPTIONS[STRATEGIES_TITLE_TO_KEY[titleKey]];
  }
  return undefined;
}
